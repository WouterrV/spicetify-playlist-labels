...
function getPlaylistsFromContents(contents) {
    ...
    function traverse(item, isRated) {
        if (item.type === 'playlist') {
            if (isRated) ratedPlaylists.push(item)
...
async function getPlaylistsExtra() {
    ...
    allPlaylists = allPlaylists.map((playlist) => ({
        ...
        isRatedPlaylist: ratedPlaylists.some(
            (ratedPlaylist) => ratedPlaylist.uri === playlist.uri,
        ),
    }))
...
async function getCachedPlaylists(db: IDBDatabase): Promise<any[]> {
    return new Promise((resolve, reject) => {
        const playlistObjectStoreRequest = db
...
async function getCachedPlaylistItems(db: IDBDatabase): Promise<any[]> {
    return new Promise((resolve, reject) => {
        const playlistItemsObjectStoreRequest = db
            .transaction('playlistItems')
...
async function clearCachedPlaylistItems(db: IDBDatabase): Promise<void> {
    return new Promise((resolve, reject) => {
        const playlistItemsObjectStoreRequest = db
            .transaction('playlistItems', 'readwrite')
            .objectStore('playlistItems')
            .clear()
        playlistItemsObjectStoreRequest.onsuccess = (event) => {
            resolve()
...
async function cachePlaylistItems(
    db: IDBDatabase,
    uriToPlaylistItems,
): Promise<void> {
    return new Promise((resolve, reject) => {
        const playlistItemsObjectStore = db
            .transaction('playlistItems', 'readwrite')
            .objectStore('playlistItems')
...
function addPlaylists(
    ...
    playlists.forEach((playlist) => {
        ...
        playlistItems.forEach((playlistItem) => {
            ...
            if (
                !trackUriToPlaylistData[trackUri].some(
                    (obj) => obj.uri === playlist.uri,
                )
            ) {
...
function addLikedTracks(trackUriToPlaylistData, likedTracks) {
    likedTracks.forEach((item) => {
        ...
        if (!trackUriToPlaylistData[trackUri])
            trackUriToPlaylistData[trackUri] = []
        if (
            !trackUriToPlaylistData[trackUri].some((obj) => obj.isLikedTracks)
        ) {
            trackUriToPlaylistData[trackUri].push({
                uri: null,
                name: 'Liked Songs',
...
export async function updateLikedTracks() {
    ...
    const trackUriToPlaylistData = {}
    const playlists = cachedPlaylists.sort(
        (a, b) => new Date(a.addedAt) - new Date(b.addedAt),
    )
    const ratedPlaylists = playlists.filter(
        (playlist) => playlist.isRatedPlaylist,
    ...
    localStorage.setItem(
        'spicetify-playlist-labels:liked-tracks-count',
        `${likedTracks.length}`,
    )
...
export async function getTrackUriToPlaylistData() {
    ...
    playlists.forEach((playlist) => {
        const cachedPlaylist = cachedPlaylists.find(
            (cachedPlaylist) => cachedPlaylist.uri === playlist.uri,
        )
        if (
            !cachedPlaylist ||
            cachedPlaylist.totalLength !== playlist.totalLength
        ) {
    ...
    const updatedPlaylistItems = await Promise.all(
        updatedPlaylists.map((playlist) => getPlaylistItems(playlist.uri)),
    )
    ...
    playlists.forEach((playlist) => {
        ...
        else uriToPlaylistItems[uri] = cachedUriToPlaylistItems[uri]
    })
    uriToPlaylistItems['likedTracks'] = cachedUriToPlaylistItems['likedTracks']

    const cachedLikedTracksCount = JSON.parse(
        localStorage.getItem('spicetify-playlist-labels:liked-tracks-count') ||
            '0',
    )
